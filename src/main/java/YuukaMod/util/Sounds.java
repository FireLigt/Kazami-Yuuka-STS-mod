package YuukaMod.util;

import static YuukaMod.Yuukamod.audioPath;

public class Sounds {
    public static String TEST_SOUND = audioPath("test.wav"); //Load audio using a given path
    public static String ding; //Load audio from audioPath based on the field name
    public static String se_enep01; //Bomb trigger sound
    public static String LLS_BOSS_BGM = "YuukaMod/audio/music/th04_15_86.ogg";
}
